...
class DocGenerator:
    ...
    def add_row(self, entity_name, class_c, method_c, block_c, line_c):
        ...
        if class_c is not None:
            ...
            row = row.replace("%CLASS_COLOR%", class_c.get_bgcolor()).replace("%CLASS_MISS%", str(class_c.get_missed()))
        else:
        ...
        row = row.replace("%METHOD_COLOR%", method_c.get_bgcolor()).replace("%METHOD_MISS%", str(method_c.get_missed()))
        row = row.replace("%BLOCK_COLOR%", block_c.get_bgcolor()).replace("%BLOCK_COV%", block_c.get_coverage())
        row = row.replace("%BLOCK_COLOR%", block_c.get_bgcolor()).replace("%BLOCK_MISS%", str(block_c.get_missed()))
        row = row.replace("%LINE_COLOR%", line_c.get_bgcolor()).replace("%LINE_COV%", line_c.get_coverage())
        row = row.replace("%LINE_COLOR%", line_c.get_bgcolor()).replace("%LINE_MISS%", str(line_c.get_missed()))
